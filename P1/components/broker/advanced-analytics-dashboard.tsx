...
export function AdvancedAnalyticsDashboard({ data, dataMode, totalData }: AdvancedAnalyticsDashboardProps) {
  ...
  const getMetrics = () => {
    if (isCSVMode && placements.length > 0) {
      ...
      const expiringIn30 = placements.filter((p) => p.daysUntilExpiry <= 30).length
      const criticalCount = placements.filter((p) => p.priorityScore >= 80).length
  ...
  const getExpiryTimeline = () => {
    ...
    return ranges.map((range) => ({
      name: range.label,
      count: placements.filter((p) => p.daysUntilExpiry >= range.min && p.daysUntilExpiry <= range.max).length,
    }))
  }
    ...
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">${(metrics.totalPremium / 1000).toFixed(0)}K</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">${(metrics.avgPremium / 1000).toFixed(1)}K</div>
