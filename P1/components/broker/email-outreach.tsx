...
Our revised quote:
• Premium: $[NEWPREMIUM] (vs $[COMPETITIVEPREMIUM])
  ...
  const handleSendEmail = async (emailBody?: string, emailSubject?: string) => {
    ...
    try {
      const response = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          to: "abhirnayak063@gmail.com",
          subject,
          body,
          templateId: selectedTemplate.id,
          policyId: selectedItem?.placement?.placementId || selectedItem?.policy?.policyHash,
        }),
      })
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-4">
          <div className="p-4 rounded-xl bg-card border border-border">
            ...
            <button
              onClick={() => setEditingTemplate({ name: "", subject: "", body: "", category: "custom", variables: [] })}
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg border transition-all ${
                      selectedItem?.policy === item || selectedItem?.placement === item
